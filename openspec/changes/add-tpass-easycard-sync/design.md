...
- 在信用卡帳戶詳情頁查看已關聯卡片的 TPASS 2.0 紀錄。
...
`linked_account_id` 可為空；若有值，必須指向同一使用者的 `CREDIT` 帳戶。每張卡可關聯一個信用卡帳戶，可隨時變更或解除。一個信用卡帳戶可關聯多張 TPASS 卡片。
...
- `GET /accounts/:id/tpass`：信用卡帳戶詳情頁使用，列出關聯 TPASS 卡片與月份摘要。
...
- 若該信用卡帳戶有關聯 TPASS 卡片，新增 TPASS 區塊。
- 區塊顯示關聯卡片、月份摘要、預估回饋與近期紀錄入口。
