...
### 方法 2: 直接讀取 PlantUML
```bash
cat backend/phase-1/2.database-schema.puml
```

### 方法 3: 使用 Grep 搜尋
```bash
grep -C 5 "entity accounts" backend/phase-1/2.database-schema.puml
```

## Schema 文件位置
- **主要 Schema**: `backend/phase-1/2.database-schema.puml`
...
根據 Schema (backend/phase-1/2.database-schema.puml:45)，
invoices 表包含以下欄位：
- id (BIGSERIAL)
